...
using PM.Core.Managers;
using PM.Core.MemoryCacher.Implementation;
using StackExchange.Redis;

namespace PM.Core.MemoryCacher;
    ...
    public RedisMemoryCacher(Logger.ILogger logger, IConfiguration configuration)
    {
        ...
        try
        {
            ConnectionMultiplexer redisConnection = ConnectionMultiplexer
                .Connect(configuration.GetValue<string>("Redis:ConnectionString"));
    ...
    public void SetString(string key, string value)
    {
        try
        {
            if(base.GetAliveStatus())
                Task.Run(()
                    => _redisDatabase.StringSet(key, value, TimeSpan.FromMinutes(5)));
        }
